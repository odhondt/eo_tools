...
# %%
# change with directory containing your partial S1 products
data_dir = "/data/S1/partial_dls"
# change with directory containing the results
out_dir = "/data/res/test-s1-core-partial-product"
if not os.path.isdir(out_dir):
    os.mkdir(out_dir)

# replace with already downloaded partial products created by
...
if not prm.is_partial or not sec.is_partial:
    raise RuntimeError("This script expects partial products with partial_download.yml.")

min_burst = max(prm.min_burst, sec.min_burst)
max_burst = min(prm.max_burst, sec.max_burst)
if max_burst < min_burst:
    raise RuntimeError(
        "Primary and secondary partial products do not share an available burst range."
    )

log.info(
    "Primary available bursts: %s to %s",
    prm.min_burst,
    prm.max_burst,
)
log.info(
    "Secondary available bursts: %s to %s",
    sec.min_burst,
    sec.max_burst,
)
log.info("Processing common burst range: %s to %s", min_burst, max_burst)

# Keep this script focused on opening partial products until core read offsets
