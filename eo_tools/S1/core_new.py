...
class S1IWSwath:
    ...
    def __init__(self, safe_path, iw=1, pol="vv", orb_dir="/tmp"):
        ...
        # First, check for Zarr store
        zarr_matches = list(self.product.glob(zarr_pattern))
        tiff_matches = list(self.product.glob(tiff_pattern))

        if zarr_matches:
            pth_raster = zarr_matches[0]
        elif tiff_matches:
            pth_raster = tiff_matches[0]
        else:
            raise FileNotFoundError("Neither Zarr nor TIFF raster file is present.")

        # If it's zipped, prepend zip:// (only makes sense for TIFFs usually)
        ...
        log.info(f"- Set up raster path {self.pth_raster}")
    ...
    def read_burst(self, burst_idx=1, remove_invalid=True):
        ...
        if Path(self.pth_raster).suffix == ".tiff":
            first_line = (burst_idx - 1) * self.lines_per_burst
            arr = read_chunk(self.pth_raster, first_line, self.lines_per_burst).astype(
                np.complex64
            )
