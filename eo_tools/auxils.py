...
def block_process(img, block_size, overlap_size, fun, *fun_args, **kwargs):
    ...
    imgout = (
        ...
        else tuple(np.zeros_like(x) for x in img)
    )
    ...
    for i in range(0, ih, block_height):
        ...
        for j in range(0, iw, block_width):
            ...
            if cnt < 5:
                ...
                print(f"Input: {i_top, i_bottom}")
                print(f"block: {b_top, b_bottom}")
                print(f"output: {o_top, o_bottom}")
                print(f"Input: {i_left, i_right}")
                print(f"block: {b_left, b_right}")
                print(f"output: {o_left, o_right}")
...
def block_process2(img, block_size, overlap_size, fun, *fun_args, **kwargs):
    """
    Block processing of a multi-channel 2-D image with an arbitrary function.
    ...
    # if any(o < 1 for o in overlap_size):
    # raise ValueError("Overlap must be greater than or equal to 1.")
    ...
    pad_left, pad_right = (
        olap_width,
        iw - block_width * (iw // block_width) + block_width,
        # iw - block_width * (iw // block_width) + block_width + olap_width,
    )
    pad_top, pad_bottom = (
        olap_height,
        ih - block_height * (ih // block_height) + block_height,
        # ih - block_height * (ih // block_height) + block_height + olap_height,
    )

    # Preallocate output image with the same shape as the input image
    img_ = np.pad(img, ((pad_top, pad_bottom), (pad_left, pad_right)))
    imgout = (
        np.zeros_like(img_)
        if not isinstance(img_, tuple)
        else tuple(np.zeros_like(x) for x in img_)
    )

    cnt = 0
    for i in range(pad_top, ih, block_height):
        for j in range(pad_left, iw, block_width):
            cnt += 1
            sl = (
            ...
            if isinstance(imgout, tuple):
                for k, output in enumerate(imgout):
                    imgout[k][sl2] = processed_block[k]
            else:
                # print(img[sl].shape)
                # print(imgout[sl2].shape), print(processed_block.shape)
                imgout[sl2] = processed_block

    return imgout[pad_top:-pad_bottom, pad_left:-pad_right]
