...
def test_process_s2_tiles(temp_output_dir):
    ...
    out_dirs = process_s2_tiles(
        s2_tiles, bands=["B1"], shp=shp, outputs_prefix=temp_output_dir, force_create=True
    )
    ...
    for out_dir in out_dirs:
        with rasterio.open(glob(f"{out_dir}/*.tif")[0]) as src:
            ...
            for key, value in expected_profile.items():
                assert profile[key] == value, f"Mismatch in {key}: expected {value}, got {profile[key]}"
            
            # Check the transform (this may need more tolerance due to floating point precision)
            expected_transform = rasterio.Affine(
                0.00011490345024196447, 0.0, -3.3854078086412236,
                0.0, -0.00011490345024196447, 48.326506645022775
            )
            assert src.transform.almost_equals(expected_transform, precision=1e-8), \
